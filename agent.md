...
### 2026-05-24 — 修复绑定后 `is_bound()` 无法识别切换后的用户 ID

**问题**：绑定成功后，发送 `帮助` 仍提示"请先完成绑定"，重新绑定显示"该 QQ 号已被绑定"

**根因**：Gensokyo 在绑定后会将用户的标识从虚拟 ID（如 `20974018`）切换为绑定值（真实 QQ 号 `2338680148`）。`is_bound()` 只检查 `_binds` 的键（虚拟 ID），用新的 QQ 号查不到匹配。

**修改**：
1. `is_bound(session_id)` — 同时检查键（虚拟ID）和值（真实QQ号）
2. `get_real_qq(session_id)` — 同时支持正向和反向查找
3. 绑定成功时除了存 `虚拟ID→QQ`，额外存一份 `QQ→QQ` 确保双向可查
4. 解绑时同步清除两份映射
